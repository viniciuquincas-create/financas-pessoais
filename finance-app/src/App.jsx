...
// ── Google Drive Sync ─────────────────────────────────────────
const GOOGLE_CLIENT_ID = "821561382667-hbrdv42b5rcmqaje30gvph56lv7cgibv.apps.googleusercontent.com";
...
async function gdriveLoad() {
  ...
  const list = await fetch(
    "https://www.googleapis.com/drive/v3/files?spaces=appDataFolder&q=name%3D%27" + GDRIVE_FILE + "%27",
...
const FIXAS_BASE = [
  { nome:"Moradia",         venc:"Dia 05", cat:"Casa" },
  { nome:"Vaga de Garagem", venc:"Dia 05", cat:"Casa" },
  { nome:"Personal",        venc:"Dia 01", cat:"Saúde" },
  { nome:"Nana (Faxina)",   venc:"Dia 10", cat:"Casa" },
  { nome:"Ana (Chef)",      venc:"Dia 10", cat:"Casa" },
  { nome:"Unimed",          venc:"Dia 10", cat:"Saúde" },
  { nome:"Vivo",            venc:"Dia 20", cat:"Apps" },
  { nome:"Consórcio",       venc:"Dia 05", cat:"Impostos" },
  { nome:"FIES",            venc:"Dia 10", cat:"Educação" },
];
...
const seedMonth = key => ({
  ...
  receitasExtra: [],
  fixas: FIXAS_BASE.map((f,i)=>({...f,id:i+1,status:"pendente",forma:"",banco:"",dataPgto:"",valor:0,extra:false})),
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"2px 0"}}>
        ...
        <button onClick={()=>setShowPaste(!showPaste)} style={{background:"rgba(124,106,247,.15)",border:"1px solid rgba(124,106,247,.25)",borderRadius:8,padding:"4px 10px",color:"#a89cf7",fontSize:10,fontWeight:600,cursor:"pointer"}}>📅 Importar agenda</button>
      </div>
    ...
    <Card style={{
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{fontSize:11,color:"#444",marginTop:1}}>{f.venc&&`${f.venc} · `}{f.cat}</div>
        </div>
...
const NAV=[{id:"dashboard",label:"Início"},{id:"plantoes",label:"Receita"},{id:"fixas",label:"Fixas"},{id:"cartoes",label:"Cartões"},{id:"variaveis",label:"Variáveis"},{id:"investimentos",label:"Invest."},{id:"analise",label:"Análise"}];
...
export default function App() {
  ...
  const [gdriveStatus,setGdriveStatus]=useState("idle"); // idle | connecting | synced | error
